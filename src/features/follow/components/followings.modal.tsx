...
import { useGetFollowings } from '../follow.hooks';
...
const FollowingsModal = ({
  ...
  const { data, isLoading, isError } = useGetFollowings(user_name!);
        ...
        <div className='flex flex-col h-full w-full rounded-2xl bg-[#262626]'>
          ...
          <div className='min-h-[200px] max-h-[400px] overflow-y-auto'>
            {isLoading ? (
              ...
              followings?.map((item) => {
                return (
                  <div key={item.following._id} className='py-2 px-4 w-full'>
                    <div className='flex items-center'>
                      <Link
                        onClick={onClose}
                        to={`/${item.following.user_name}`}
